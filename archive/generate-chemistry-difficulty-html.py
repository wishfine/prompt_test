# -*- coding: utf-8 -*-
"""
@File    : generate-chemistry-difficulty-html.py
@Description:
    从化学难度评级结果 JSONL 中按 5 档难度进行采样，生成便于人工观察与标注验证的 HTML 可视化网页。
...
def split_urls(urls: str) -> List[str]:
    """兼容 stem_pic_url / analysis_pic_url 中逗号拼接多个图片 URL 的情况。"""
    if not urls:
        return []
    if isinstance(urls, list):
        return [str(u).strip() for u in urls if str(u).strip()]
    return [u.strip() for u in str(urls).split(',') if u.strip()]
...
__QUESTION_CARDS_PLACEHOLDER__
    ...
    function loadAnnotations() {
        try {
            return JSON.parse(localStorage.getItem('chemistry_difficulty_annotations') || '{}');
        } catch { return {}; }
    }

    function saveAnnotations(annotations) {
        localStorage.setItem('chemistry_difficulty_annotations', JSON.stringify(annotations));
        updateStats();
    }
    ...
    function exportJSONL() {
        ...
        a.href = url;
        a.download = 'chemistry_difficulty_human_annotations.jsonl';
    ...
    function exportTXT() {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = 'chemistry_difficulty_review_report.txt';
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
...
"""
        for idx, item in enumerate(items, 1):
            ...
            stem_urls = split_urls(item.get('stem_pic_url', ''))
            analysis_urls = split_urls(item.get('analysis_pic_url', ''))
            stem_url = ','.join(stem_urls)
            analysis_url = ','.join(analysis_urls)
...
'''
                for img_url in stem_urls:
                    cards_html += f'''                        <img src="{html.escape(img_url)}" alt="题干图示" onerror="this.outerHTML='<div style=\'color:#999;font-style:italic\'>题干图示加载失败，可能为局域网内网地址</div>'">
'''
                cards_html += '''                    </div>
                </div>
'''
...
"""
            ...
            if analysis_urls:
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <div class="image-container">
'''
                for img_url in analysis_urls:
                    cards_html += f'''                        <img src="{html.escape(img_url)}" alt="解析图示" onerror="this.outerHTML='<div style=\'color:#999;font-style:italic\'>解析图示加载失败</div>'">
'''
                cards_html += '''                    </div>
                </div>
'''
...
def main():
    parser = argparse.ArgumentParser(description="初中化学题目难度评级数据可视化 HTML 网页生成工具")
    parser.add_argument("-i", "--input", type=str, default="chemistry_difficulty_rated_results.jsonl",
                        help="化学评级跑出的输出 JSONL 路径")
    parser.add_argument("-o", "--output", type=str, default="chemistry_difficulty_rated_validation.html",
                        help="生成的可视化网页保存路径")

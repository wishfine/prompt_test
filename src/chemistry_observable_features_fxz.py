...
# 正式模型输出固定为 V5 十七项，避免继续增加 Schema 负担并干扰
# 五档边界判断。
...
# 切换的回答规则。保持单字段、有限枚举，避免额外字段增加 Schema 负担。
RULE_FAMILIES = {
    "教材事实直接匹配",
    "分类或概念辨析",
    "化学用语书写",
...
def normalize_observable_features(
    ...
    known_feature_fields = set(OBSERVABLE_FEATURE_FIELDS)
    ...
    for field in ("curriculum_topics", "longest_solution_chain"):
        values = normalized.get(field)
        if not isinstance(values, list):
            continue
        deduped: List[Any] = []
...
def validate_observable_features(features: Any) -> Dict[str, Any]:
    ...
    _validate_single_enum(validated, "solution_topology", SOLUTION_TOPOLOGIES)
    _validate_single_enum(
        validated, "experiment_task_structure", EXPERIMENT_TASK_STRUCTURES
    )
    ...
    _validate_unique_enum_list(
        validated, "curriculum_topics", CURRICULUM_TOPICS, allow_empty=False
    )
    _validate_single_enum(
        validated, "parallel_task_relation", PARALLEL_TASK_RELATIONS
    )
    _validate_single_enum(
        validated,
        "reaction_structure",
    ...
    _validate_single_enum(
        validated, "visual_task_structure", VISUAL_TASK_STRUCTURES
    )
    _validate_single_enum(
        validated,
        "graph_table_operation",
    ...
    _validate_single_enum(
        validated, "error_analysis_operation", ERROR_ANALYSIS_OPERATIONS
    )
    _validate_single_enum(
        validated,
        "new_information_operation",
    ...
    if (
        validated["error_analysis_operation"] != "无误差分析"
        and validated["experiment_operation"] == "无"
    ):
...
def derive_observable_metrics(
    ...
    topics = [topic for topic in validated["curriculum_topics"] if topic != "U_OTHER"]
    units = sorted({topic.split("-", 1)[0] for topic in topics})
    span_type = "跨单元" if len(units) >= 2 else ("同单元跨课题" if len(topics) >= 2 else "单一课题")
    parallel = validated["parallel_task_relation"]
    coupling_type = "单一课题" if span_type == "单一课题" else (f"{span_type}并列" if parallel in {"同一规则下多个对象", "不同规则的独立任务"} else f"{span_type}耦合")
    def valid(values: List[str]) -> List[str]:
        return [value for value in values if value not in OBSERVABLE_FALLBACK_LABELS]
    effective_groups = [group for group in validated["task_groups"] if group["task_type"] != OBSERVABLE_FALLBACK_LABEL_BY_FIELD["task_type"]]
    calculations = valid(validated["calculation_operations"])
    advanced = {"组分消元或组成不变量", "差量", "多反应定量关系", "联立", "范围或分类计算"}
    return {
        "longest_chain_steps": len(validated["longest_solution_chain"]),
        "effective_task_count": sum(group["count"] for group in effective_groups),
        "task_group_count": len(effective_groups),
        "rule_family_count": len(valid(validated["rule_families"])),
        "curriculum_topic_count": len(topics),
        "curriculum_unit_count": len(units),
        "curriculum_span_type": span_type,
        "curriculum_coupling_type": coupling_type,
        "curriculum_span_summary": f"{coupling_type}（" + "、".join(topics) + "）",
        "condition_operation_count": len(valid(validated["condition_operations"])),
        "representation_operation_count": len(valid(validated["representation_operations"])),
        "evidence_operation_count": len(valid(validated["evidence_operations"])),
        "calculation_operation_count": len(calculations),
        "advanced_calculation_operations": [value for value in calculations if value in advanced],
        "has_task_dependency": bool(len(validated["longest_solution_chain"]) >= 2 and parallel in {"单一答题目标", "共享同一化学模型的关联任务"}),
        "solution_topology": validated["solution_topology"],
        "experiment_task_structure": validated["experiment_task_structure"],
    }

...
第一版融合 Prompt 过度压缩，三次在线最终结果只有 `102/133`、`101/133`、`101/133`，主要表现为中等、拔高和压轴题系统性判低。本版已把 Prompt 从约 15 KB 恢复到约 35 KB，保留完整教师边界、特征判定细则、五档检核和 9 个互补 few-shot；仍明显短于约 53 KB 的旧 V7。用上述三次文件的真正原始字段 `difficulty_rating_raw` 回放修订后处理，结果为 `106/133`、`106/133`、`104/133`，没有新增误改，原始等级波动 20 道降为最终波动 14 道，后处理新增波动为 0。新 Prompt 本身仍须在服务器连续在线跑三次确认。

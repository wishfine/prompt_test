...
class JuniorChemistrySchemaTests(unittest.TestCase):
    ...
    def test_prompt_documents_every_runtime_enum(self):
        ...
        self.assertIn("【Case 27：材料类别的生活常识干扰】", prompt)
        self.assertIn("【Case 57：石青分阶段受热】", prompt)
        self.assertIn("【Case 62：锌与两种盐溶液反应后的滤液滤渣】", prompt)
        self.assertIn("【Case 97：单一高难实验或计算 vs 多类型高难任务耦合】", prompt)
        self.assertIn("【Case 52：装置作用、顺序、改进与计算联合】", prompt)
        self.assertNotIn("【Case 98", prompt)
        case_numbers = [int(value) for value in re.findall(r"【Case (\d+)", prompt)]
        self.assertEqual(case_numbers, list(range(1, 98)))
        self.assertNotIn("同档锚点", prompt)
        self.assertNotIn("补充边界", prompt)
        self.assertIn("【Case 17：利用化合价代数和求化合价或化学式】", prompt)
        self.assertIn("【Case 64：常见化学式再认 vs 化合价关系应用】", prompt)

...
2. 基础题不再自动降送分；送分升基础只看公式计算、明确作图/连接/实验动作、递进推理或真实生活映射，不能因出现“透镜”“电路”等学科名词触发。
3. 基础升中等：至少一个强信号（控制变量/故障、多组归纳、多公式、图像反推、真实跨模块），或至少两个弱信号且存在多层连续推理；独立题、单图方向标注、结构补空和基础读数排除。
...
当前离线回放同一批 133 条原始模型 JSON 时，旧后处理为 82/133 完全一致、MAE 0.406；修订规则为 87/133、MAE 0.361。该回放只验证后处理止损，Prompt 效果仍须在线重跑确认。

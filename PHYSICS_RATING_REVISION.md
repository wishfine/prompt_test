...
- 恢复旧 V7 中可泛化的送分白名单、中等弱保护和低计算高建模通道，不恢复题目专属关键词。
...
| 3659089396571848704 | 中等 | 光的反射标准实验 | 实验流程和数据归纳不压基础 |
...
2. 基础题不使用宽泛特征自动降送分；Prompt 恢复 V7 教材原型送分白名单，同时排除生活应用和规范电磁/电路作图。
3. 基础升中等：控制变量、多组归纳等强实验信号优先于“独立小问”拦截；标准规则作图保护在基础。“跨模块”必须有至少三个过程支撑信号或其他强证据，避免把并列概念当真实综合。
4. 中等升拔高：3-5 步需两个核心高阶信号且至少一个真正迁移信号（跨模块、逆向/临界、图像反推、多式联立、多变量耦合）；双状态、递进多问或普通方案设计不能单独构成强迁移。6-8 步需一个核心高阶信号且不是独立并列题。
5. 拔高升压轴：至少三个核心高阶信号和一个强压轴信号。项目/控制题还必须有可行性/边界/分类/方案比较等强验证、至少两个高阶模型信号，以及复杂计算、临界推理或 6 步以上证据。
6. 拔高降中等只在 `1-2步` 且无高阶信号时触发；`3-5步` 不因“未抽取到高阶信号”降档。压轴降拔高仍独立判断，缺少“主动升压轴”证据不会自动降档。
...
当前离线回放 `lite_physics_v2_v7_recovery.jsonl` 的 133 条原始模型 JSON 时，文件中旧后处理为 87/133 完全一致、MAE 0.353；本版 V7 通用保护层回放为 95/133、MAE 0.293，8 次规则调整全部改善、无新增误调。该回放只验证后处理，修订后 Prompt 效果仍须在线重跑确认。

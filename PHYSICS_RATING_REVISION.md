...
- 2026-07-15 不使用难度说明/分维标准，改为逐题审计每档 10 道真实样本；主判定轴调整为「直接识别、显性应用、常规分析、决定性转换、全链耦合」。完整审计见 `PHYSICS_ACTUAL_SAMPLES_AUDIT.md`。
- 删除 Prompt 中得分率 `P` 的所有表述；输入确定不会提供该字段。

### few-shot 选择表

| question_id | 教师标签 | 题目摘要 | 选择原因 / 校准边界 |
| --- | --- | --- | --- |
| 2711099389137764352 | 容易 | 常见电学量估测 | 四个短选项不会自动排除送分 |
| 3659087202128773120 | 较易 | 菜刀刀刃变薄增大压强 | 简单生活映射至少基础；与完整规范作图共校准送分/基础边界 |
| 3650594178713079808 | 较易 | 油条制作的四个独立概念选项 | 独立知识点不因数量或表面跨模块升中等 |
| 2139834545681850370 | 中等 | 摩擦力多条件高密度辨析 | 无计算但需区分充分/必要条件和反例，不压成基础 |
...
| 2293516624119222272 | 较难 | 灯泡/变阻器两条图线身份反推 | 一个决定性转换可以达到拔高，不要机械凑高阶特征数 |
| 3385313576457912320 | 困难 | 多开关灯泡电路与功率边界 | 多状态等效电路、前问复用和多安全约束形成完整链 |
